...
impl Runner {
    ...
    pub async fn run(self) -> Result<()> {
        while *self.depth.lock() < self.opts.depth.unwrap() {
            let previous_nodes = self.get_previous_nodes();
            let (_root_progress, progresses) = self.setup_progress(&previous_nodes);
            let mut handles = Vec::new();
            for previous_node in &previous_nodes {
                let handle = self.process_node(previous_node, &progresses);
                handles.extend(handle);
            }
            self.wait_for_all_handles(handles).await?;
            *self.depth.lock() += 1;
        }
        Ok(())
    }

    fn get_previous_nodes(&self) -> Vec<Arc<Mutex<TreeNode<TreeData>>>> {
        self.tree
            .lock()
            .get_nodes_at_depth(self.depth.lock().clone())
    }

    fn setup_progress(
        &self,
        previous_nodes: &Vec<Arc<Mutex<TreeNode<TreeData>>>>,
    ) -> (MultiProgress, HashMap<String, ProgressBar>) {
        ...
        for previous_node in previous_nodes {
            ...
            let pb = root_progress
                .add(ProgressBar::new((self.words.len()) as u64))
                .with_style(
        ...
        for (i, chunk) in self.chunks.iter().enumerate() {
            ...
            let handle = tokio::spawn(async move {
                    ...
                    < chunk.len()
                {
                    ...
                    let sender = client::get_sender(&opts, &url, &client::build(&opts).unwrap());
                    ...
                    match response {
                        Ok(mut response) => {
                            ...
                            let filtered =
                                filters::check(&opts, &text, status_code, t1.elapsed().as_millis());

                            if filtered {
                                let additions = filters::parse_show(&opts, &text, &response);
    ...
    async fn wait_for_all_handles(&self, handles: Vec<JoinHandle<()>>) -> Result<()> {
