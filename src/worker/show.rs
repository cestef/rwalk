...
type ShowFunction = Box<dyn Fn(&RwalkResponse) -> String + Send + Sync>;

fn boxed_fn(f: fn(&RwalkResponse) -> String) -> ShowFunction {
    Box::new(f)
}

lazy_static! {
    pub static ref SHOW_REGISTRY: HashMap<String, ShowFunction> = {
